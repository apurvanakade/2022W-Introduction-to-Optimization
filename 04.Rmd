...
Given a standard linear program \@ref(eq:standard-lp), we can form an linear program called **dictionary** of the following form by introducing **slack variables** $x_{n+1}, \dots, x_{n + m}$:
...
2. *Find leaving variable:*
    ...
    2. Set $\ell = \mathrm{arg}\max_{i \in S} \left(\dfrac{b_i}{a_{ie}}\right)$.
3. *Pivot:*
...
It attempts to find a sequence of pivots on the dictionary \@ref(eq:dictionary-definition) and make it feasible. 
