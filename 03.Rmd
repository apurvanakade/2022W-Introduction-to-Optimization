...
The first and arguably the most important trick in the simplex method is to introduce *slack variables*  $x_3$ and $x_4$, one for each constraint, as shown below, and create a new linear program called a *dictionary* which is equivalent to \@ref(eq:simplex-example)
...
Using these new variables, the points of intersection can be written as intersections of the lines $x_i = 0$ and $x_j = 0$, as $i$ and $j$ vary over the index set $\{1, 2, 3, 4\}$. 
